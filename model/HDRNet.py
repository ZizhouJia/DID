...
class Guide(nn.Module):
    ...
    def __init__(self, mode="PointwiseNN",inc=3):
        ...
        if mode == "PointwiseNN":
            ...
            self.conv1 = conv_block(inc, 16, kernel_size=1, padding=0, is_BN=True)
            self.conv2 = conv_block(16, 1, kernel_size=1, padding=0, activation=nn.Tanh())

        elif mode == "PointwiseCurve":
            # ccm: color correction matrix
            self.ccm = nn.Conv2d(inc, 3, kernel_size=1)
...
class Transform(nn.Module):
    ...
    def forward(self, coeff, full_res_input):
        if(full_res_input.size(1)==4):
            R = torch.sum(full_res_input * coeff[:, 0:4, :, :], dim=1, keepdim=True) #+ coeff[:, 3:4, :, :]
            G = torch.sum(full_res_input * coeff[:, 4:8, :, :], dim=1, keepdim=True) #+ coeff[:, 7:8, :, :]
            B = torch.sum(full_res_input * coeff[:, 8:12, :, :], dim=1, keepdim=True) #+ coeff[:, 11:12, :, :]

...
class HDRNet_solver(solver.common_solver):
    def __init__(self,saveimg_path):
        super(HDRNet_solver,self).__init__()
        self.images=[]
        self.counts=[]
        self.loss=[]
        self.psnrs=[]
    ...
    def get_default_config():
        ...
        config["mode"]="HDRNET"
        config["learning_rate_decay_epochs"]=[100]
        # config["saveimg_path"]="result_images/"+self.config["task_name"]
        return config
    ...
    def load_config(self):
        super(HDRNet_solver,self).load_config()
        self.mode=self.config["mode"]
        self.learning_rate_decay_epochs=self.config["learning_rate_decay_epochs"]
        # self.saveimg_path=self.config["saveimg_path"]

    def forward(self,data):
        x,y,ratio_mat,id=data
        x=x.cuda()
        y=y.cuda()
